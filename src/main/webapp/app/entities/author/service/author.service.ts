...
export class AuthorService {
  ...
  protected convertDateFromClient(author: IAuthor): IAuthor {
    return Object.assign({}, author, {
      birth: author.birth?.isValid() ? author.birth.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.birth = res.body.birth ? dayjs(res.body.birth) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((author: IAuthor) => {
        author.birth = author.birth ? dayjs(author.birth) : undefined;
      });
    }
    return res;
  }
